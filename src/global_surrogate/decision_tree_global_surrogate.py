...
class DecisionTreeSurrogate:
    _black_box_model: BaseModel
    _surrogate_model: DecisionTreeClassifier


    def __init__(self, black_box_model: BaseModel, params: dict):
        self._black_box_model = black_box_model
        self._surrogate_model = DecisionTreeClassifier(**params)

    def fit(self, X_train, y_train):
        assert len(y_train.shape) == 1, "y_train should be 1D"

        _y_train = self._black_box_model.predict(X_train)
        self._surrogate_model.fit(X_train, _y_train)
        ...
        def recurse(node, depth, parent_rule):
            ...
            if tree_.feature[node] != _tree.TREE_UNDEFINED:
                ...
                if sample_counts < 500:
                    return
                print(f"{indent}Leaf({node}): {parent_rule} -> Predict = {[round(val, 2) for val in predictions]}, Samples per class = {predictions}, Total samples = {sample_counts}")
